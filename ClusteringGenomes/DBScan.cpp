...
std::vector<int> DBScan::getClusters(std::vector<std::vector<std::string>>& myRows) {
  ...
  distTable.resize(myRows.size(), std::vector<double>(myRows.size(), 0.0));

  std::cout << "Entering db scan" << std::endl;

  //Initial differentation of core, edge, noise points
  for (int i = 0; i < myRows.size(); i++) {
    std::cout << "I: " << i << std::endl;
    for (int j = i + 1; j < myRows.size(); j++) {
      distTable[i][j] = m_distanceMetric->getDistance(myRows[i], myRows[j]);
      distTable[j][i] = distTable[i][j];
      if (distTable[i][j] < m_epsilon) {
        numNeighbors[i]++;
        numNeighbors[j]++;
      }
    }
  ...
  std::cout << "Finished initial diff or core, edge, and noise points." << std::endl;
  ...
  for (int i = 0; i < myRows.size(); i++) {
    if (myTypes[i] == 1) {
      ...
      for (int j = 0; j < myRows.size(); j++) {
        if (myTypes[j] == 2) {
          double potentialDistance = distTable[i][j];
  ...
  std::cout << "Finished separation of edge and noise points." << std::endl;
  ...
  for (int i = 0; i < myRows.size(); i++) {
    if(myTypes[i] == 2){
      ...
      for (int j = i + 1; j < myRows.size(); j++) {
        if (myTypes[j] == 2 && (distTable[i][j] < m_epsilon)) {
          neighboringCorePts[i].push_back(j);
        }
      }
    }
  }
  std::cout << "Finished finding neigboring core points." << std::endl;
